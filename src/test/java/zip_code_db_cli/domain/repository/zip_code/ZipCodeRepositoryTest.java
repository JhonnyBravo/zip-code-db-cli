...
public class ZipCodeRepositoryTest {
    public static class テーブルが空である場合 {
        ...
        private ConnectionRepository cs;
        ...
        public void setUp() throws Exception {
            ...
            cs = new ConnectionRepositoryImpl();
            zcr = new ZipCodeRepositoryImpl();

            try (Connection connection = cs.getConnection(attr)) {
                final List<ZipCode> recordset = zcr.findAll(connection);

                if (recordset.size() > 0) {
                    zcr.deleteAll(connection);
                }
            }
        }
        ...
        public void create実行時にレコードを登録できること() throws Exception {
            ...
            try (Connection connection = cs.getConnection(attr)) {
                final boolean status = zcr.create(connection, contents);
    ...
    public static class テーブルが空ではない場合 {
        ...
        public void setUp() throws Exception {
            ...
            try (Connection connection = cs.getConnection(attr)) {
                ...
                zcr.create(connection, contents);
            }
        }
        ...
        public void deleteAll実行時にテーブルを空にできること() throws Exception {
            try (Connection connection = cs.getConnection(attr)) {
                final boolean status = zcr.deleteAll(connection);
                assertThat(status, is(true));
